...
int main(int argc , char* argv[]){
	...
	if (!client.Start())
		...
		//VideoCapture cap("../data/5n.m4v");
	//VideoCapture cap("../data/noite.mp4"); // video teste
	VideoCapture cap(1); // captura
	...
	*/
	...
	Scalar high_red = cvScalar(120 * HUE_MULTIPLIER, 50 * SATURATION_MULTIPLIER, 100 * VALUE_MULTIPLIER); //Faróis
	...
	for (;;){
		...
		*/
		...
		////////////////////////comentado para teste de dia com a cam!!!
		/*
		...
		*/
		...
		rectangle(img_inRange, Rec_exesquerda, Scalar(255, 0, 0), 0, 0, 0);
		rectangle(img_inRange, Rec_midesquerda, Scalar(255, 0, 0), 0, 0, 0);
		rectangle(img_inRange, Rec_centro, Scalar(255, 0, 0), 0, 0, 0);
		rectangle(img_inRange, Rec_middireita, Scalar(255, 0, 0), 0, 0, 0);
		rectangle(img_inRange, Rec_exdireita, Scalar(255, 0, 0), 0, 0, 0);
		...
		if (i > 5)
		{
			...
			//writeResult = SP->WriteDatabin(&msg, 1);
			//Sleep(100);
			//char incomingData[3] = "";
			...
			int faroisAuto  = ( (int) incomingData[0] & 0b00000001);
			int faroisDia = ((((int) incomingData[0] >> (1)) & 0b00000001));
			int faroisMinimos = ((((int)incomingData[0] >> (2)) & 0b00000001));
			int faroisMaximos = ((((int)incomingData[0] >> (3)) & 0b00000001));
			int faroisI2CError = ((((int)incomingData[0] >> (4)) & 0b00000001));
			int faroisVideoError = ((((int)incomingData[0] >> (5)) & 0b00000001));
